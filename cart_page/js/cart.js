    ...
    else
    {
        ...
        { 
            tbody.innerHTML+=`<tr>
                                ...
                                <td class="pt-2 pb-2"><img src=${cart[i].img}></td>
                                <td><input type="number" id="inputPassword5" class="form-control quantity w-100" value="${cart[i].orderedQuantity}"></td>
                                <td class="text-start"><button class="increaseBtn btn btn-success">+</button></td>
                                <td>${cart[i].price}$</td>
                                <td>
                                    <button class="btn btn-dark" onclick="removeProduct(${cart[i].id})">remove</button>
                                </td>
                            </tr>`
                            total+=(Number(input[i].value)) * (Number(cart[i].price))       
        }
            priceDiv.innerHTML=`Total Price: $${total}`
                let sum=0
...
function removeProduct(productID)
{
    ...
    for(let i=0;i<newCart2.length;i++)
        { 
            tbody.innerHTML+=`<tr>
                                ...
                                <td><img src=${newCart2[i].img}></td>
                                 <td><input type="number" id="inputPassword5" class="form-control quantity w-100" value="${newCart2[i].orderedQuantity}"></td>
                                <td class="text-start"><button class="increaseBtn btn btn-success">+</button></td>
                                <td>${newCart2[i].price}$</td>
                                <td>
                                    <button class="btn btn-dark" onclick="removeProduct(${newCart2[i].id})">remove</button>
                                </td>
                            </tr>`
                            var input=document.getElementsByClassName('quantity');
        ...
        priceDiv.innerText=``
        priceDiv.innerHTML=`Total Price: $${all}`
        ...
        for(let x=0;x<newCart2.length;x++)
            {
                btn[x].addEventListener('click',function(e){
                    ...
                    priceDiv.innerText=``
                    priceDiv.innerHTML=`Total Price: $${sum}`
                })
            }  
